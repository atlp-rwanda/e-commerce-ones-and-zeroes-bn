...
class ProductController {
  static async getAllProducts(req: Request, res: Response) {
    try {
      const allProducts = await db.ProductMig.findAll();
      return res
        .status(200)
  ...
  static async getSingleProduct(req: Request, res: Response) {
    try {
      const singleProduct = await db.ProductMig.findOne({
        where: {
  ...
  static async updateProduct(req: Request, res: Response) {
    try {
      ...
      const { name, description, price, quantity, discount, expirydate } =
        req.body;
      if (name) {
      ...
      if (expirydate) {
        singleProduct.expirydate = expirydate;
      }
  ...
  static async updateImage(req: Request, res: Response) {
    try {
      ...
      if (req.file) {
        if(singleProduct.imageurl.length>4){
          return res.status(400).json({
            status: 'error',
            message: 'you reached the maximum number of images product must have',
          });
        }
        singleProduct.imageurl = [...singleProduct.imageurl, req.file.path];
      }
  ...
  static async removeProductImage(req: Request, res: Response) {
    ...
    try {
      // Find the product by ID
      const product = await db.ProductMig.findOne({ where: { productId } });
      ...
      const updatedImages = product.imageurl.filter(
        (url: string) =>
          url.trim().toLowerCase() !== imageurl.trim().toLowerCase(),
      );

      // Check if any image was removed
      if (updatedImages.length === product.imageurl.length) {
        return res.status(400).json({
          status: 'Bad Request',
