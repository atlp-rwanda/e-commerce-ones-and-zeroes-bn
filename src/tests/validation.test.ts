...
describe('validations', () => {
    describe('validateUpdatePassword', () => {
        ...
        test('should return validation error if newPassword does not meet criteria', () => {
            ...
            expect(res.json).toHaveBeenCalledWith({ status: 'fail', message: 'validation error', error: 'Minimum password length is 6, with at least one number, one uppercase letter, and one special character' });
        ...
        test('should return validation error if verifyNewPassword does not match newPassword', () => {
            ...
            validations.validateUpdatePassword(req, res, next);
            expect(res.status).toHaveBeenCalledWith(422);
            expect(res.json).toHaveBeenCalledWith({ status: 'fail', message: 'validation error', error: '"verifyNewPassword" must be [ref:newPassword]' });
            expect(next).not.toHaveBeenCalled();
        });
        ...
        test('should return validation error if any required field is missing', () => {
            ...
            validations.validateUpdatePassword(req, res, next);
            expect(res.status).toHaveBeenCalledWith(422);
            expect(res.json).toHaveBeenCalledWith({ status: 'fail', message: 'validation error', error: '"password" is required' });
            expect(next).not.toHaveBeenCalled();
        });
    });
});